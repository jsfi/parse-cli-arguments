...
module.exports = function(args, settings) {
    for (let i = 0, len = args.length; i < len; i++) {
        ...
        if (_.startsWith(arg, settings.flagSymbol)) {
            prop = getType(arg, settings.flags, settings, 'flag', settings.flagSymbol);
            ...
            argProps = argParts.map(el => getType(el, settings.aliases, settings, 'alias', settings.aliasSymbol)
            ).filter(_.identity);
...
function getType(arg, check, settings, property, prefix) {
